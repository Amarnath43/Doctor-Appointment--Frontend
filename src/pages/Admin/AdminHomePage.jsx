...
const AdminHomePage = () => {
  ...
  try {
    const { data } = await AxiosInstances.get('/admin/dashboard-summary');
    if (data.success) {
  ...
  fetchDashboardSummary();
    ...
    useEffect(() => {
        const fetchFilteredAppointments = async () => {
            ...
            try {
                const { data } = await AxiosInstances.get(`/admin/today-appointments?status=${filter}&page=${currentPage}&limit=10`);
                if (data.success) {
